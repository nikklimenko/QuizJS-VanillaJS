...
export class Choice {
    ...
    async init() {
        ...
        if (userInfo) {
            try {
                const result = await CustomHttp.request(config.host + '/tests/results?userId=' +
                    userInfo.userId)
        ...
        if (this.quizzes && this.quizzes.length > 0) {
            this.quizzes.forEach(quiz => {
                ...
                if (result) {
                    ...
                    choiceOptionResultElement.innerHTML = '<div>Результат</div><div>' + result.score + '/' + result.total
                    '</div>';
                ...
                choiceOptionImageElement.setAttribute('alt', 'arrow right');
